...
void dfs(int **adjMat, int nodes, int *vis, int node, int *index, int *topo) {
    vis[node] = 1;
    int i;

    for(i = 1; i < nodes; i++) {
        if(i != node) {
            if(adjMat[node][i] == 1 && !vis[i])dfs(adjMat,nodes,vis,i,index,topo);
        }
    }
...
int main() {
    ...
    int *topo = (int *)malloc(sizeof(int) * nodes);
    int index = 0;
    int i,j;
    for(i = 0; i < nodes; i++) {
    ...
    accept(adjMat, nodes);
    for(i = 1; i < nodes; i++) {
        if(!vis[i])dfs(adjMat, nodes, vis,i,&index,topo);
    }
